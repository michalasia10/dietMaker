...
from features.user.meal import Meal
...
def add_recipe(db: Session, timestamp: int, meal_id: int, recipe_id: int, user_id: int):
    ...
    meal = get_by_id(db, Meal, meal_id).first()
    dailyMeal = db.query(DailyMealPlan).filter(DailyMealPlan.date == reversed_timestamp, DailyMealPlan.meal == meal).first()
    if not dailyMeal:
        dailyMealId = simple_object_creator(db, DailyMealPlan,
                                            date=reversed_timestamp,meal=meal).id
    else:
